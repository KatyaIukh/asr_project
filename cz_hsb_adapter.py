...
# Загрузка и объединение адаптера высокоресурсного языка
model = PeftModel.from_pretrained(model, "whisper-small-czech-lora")
...
# Конфигурация LoRA для адаптера
...
training_args = Seq2SeqTrainingArguments(
    output_dir="whisper-small-sorbian-lora-cz",
...
def evaluate_per_sample(model, dataset, processor):
    """Оценка WER для каждого примера отдельно"""
    model.eval()
    wers = []
    predictions_list = []
    references_list = []
...
# Сохраняем модель
trainer.save_model("whisper-small-cz-sorbian-lora")
